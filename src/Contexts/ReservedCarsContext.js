...
export const ReservedCarsProvider = ({ children }) => {
	const [shouldRedirectAndRefresh, setShouldRedirectAndRefresh] =
		useState(false)
	...
	const [reservedCars, setReservedCars] = useState(() => {
		...
		if (
			...
			Date.now() - storedData.timestamp <= reservedCarsTime
		) {
	...
	const removeCar = indexToRemove => {
		setReservedCars(prevCars =>
			prevCars.filter((_, index) => index !== indexToRemove)
		)
	}
	...
	return (
		<ReservedCarsContext.Provider
			value={{ reservedCars, addCarToReservation, removeCar, isCarReserved }}>
			{children}
		</ReservedCarsContext.Provider>
	)
}
