...
import {
	StyledButton,
	StyledLink,
} from "./ReservationSearch/ReservationSearch.styles"
...
function ReservationSearch({ showLanguage = true, onSearch = () => {} }) {
	...
	const handleButtonClick = () => {
		...
		if (clickedOne) {
			isRefundCitySelected =
				selectedRefundCity && selectedRefundCity.value === "4"
	...
	return (
		<StyledComponent>
			...
			<DatePicker
				useDate={0}
				onClick={() => showPickupDatePanelHandler(0)}
				selectedDate={pickupDate}
				onDateChange={handlePickupDateChange}
			/>
			<DatePicker
				useDate={1}
				onClick={() => showReturnDatePanelHandler(1)}
				selectedDate={returnDate}
			...
			<StyledButton onClick={handleButtonClick}>
				<StyledLink>{t("button")}</StyledLink>
			</StyledButton>
			<ToastContainer />
		</StyledComponent>
	)
