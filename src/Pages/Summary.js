		...
		validationSchema: Yup.object({
			nameSurname: Yup.string()
				...
				.matches(
					...
					t("validationSummary.summaryNameSurname.summaryMatches")
				),
			phoneNumber: Yup.string()
				.required(t("validationSummary.summaryPhoneNumber.summaryRequired"))
				.matches(
					/^\+?(\d{8,15})$/,
					t("validationSummary.summaryPhoneNumber.summaryMatches")
				),
			email: Yup.string()
			...
			zipCode: Yup.string()
				...
				.matches(
					/^\d{2}-\d{3}$/,
					t("validationSummary.summaryZipCode.summaryMatches")
				),
			city: Yup.string().required(t("validationSummary.summaryCity")),
			country: Yup.string().required(t("validationSummary.summaryCountry")),
			nip: Yup.string().test(
				"isCompany",
				t("validationSummary.summaryNip"),
				function (value) {
					return this.parent.isCompany ? !!value : true
				}
			),
			regon: Yup.string().test(
				"isCompany",
				t("validationSummary.summaryRegon"),
				function (value) {
					return this.parent.isCompany ? !!value : true
				}
			),
			carAgreement: Yup.boolean().oneOf(
				[true],
				t("validationSummary.summaryCarAgreement")
			),
			personalDataAgreement: Yup.boolean().oneOf(
				[true],
				t("validationSummary.summaryPersonalDataAgreement")
			),
		}),
		onSubmit: values => {
			setIsModalOpen(true)
	...
	return (
		<>
			...
			<ReservationSummaryPanel
				handleValidationAndSubmit={formik.handleSubmit}
			/>
