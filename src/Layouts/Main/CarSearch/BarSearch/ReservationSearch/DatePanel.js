...
import { StyledButtonDate, StyledDatePanel, StyledDateTime, StyledButtonContainer } from './ReservationSearch.styles';
...
function DatePanel({ isOpen, closeDate, useDate, onPickupDateChange, onReturnDateChange }) {
    ...
    const getCurrentFormattedDate = () => {
        ...
        const monthName = t(`${new Intl.DateTimeFormat(i18n.language, { month: 'long' }).format(now).toLowerCase()}`);
    ...
    useEffect(() => {
      if ((selectedPickupState || selectedReturnState) && dateSelected) {
          if (datetimeRef.current && datetimeRef.current.state.currentView !== "time") {
        ...
        <StyledDatePanel className={isTimeView ? "" : "hide-buttons"} >
            ...
            <div className="date-picker-container">
                ...
                ref={datetimeRef}
                    ...
                    value={useDate === 0 ? selectedPickupState : useDate === 1 ? selectedReturnState : null}
                    onChange={(newDate) => {
                      setDateSelected(true);
                        if (useDate === 0) {
                            setSelectedPickupState(newDate);
                            if (newDate.isAfter(selectedReturnState)) {
                              const newReturnDate = newDate.clone().locale('pl').add(1, 'day');
            ...
            <StyledButtonContainer>
                <StyledButtonDate onClick={closeDate} style={{ marginRight: '20px', cursor: 'pointer' }}>{t('buttons.cancel')}</StyledButtonDate>
                <StyledButtonDate onClick={handleConfirmClick} style={{ cursor: 'pointer' }}>{t('buttons.confirm')}</StyledButtonDate>
            </StyledButtonContainer>
        </StyledDatePanel>
        </>
    );
