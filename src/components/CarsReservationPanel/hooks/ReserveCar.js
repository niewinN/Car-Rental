...
export const useHandleReserve = () => {
	...
	const { reservedCars, addCarToReservation, removeCar, isCarReserved } =
		useContext(ReservedCarsContext)

	const handleReserve = car => {
		const currentTime = new Date().getTime()
		const isRefundCitySelected = clickedOne
			? selectedRefundCity && selectedRefundCity.value !== "1"
			: true
		...
		if (isCarReserved(car.id)) {
			const index = reservedCars.findIndex(
			...
			addCarToReservation(
				car,
				selectedPickupState.format("YYYY-MM-DD HH:mm"),
				selectedReturnState.format("YYYY-MM-DD HH:mm"),
				selectedCity
			)
		}
		return true
