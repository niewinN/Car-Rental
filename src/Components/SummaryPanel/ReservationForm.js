...
import { FormContainer, FormBox, Section, LabelNip, Title, Label, Input, LabelNipLink, ErrorMessage, CheckboxContainer, CompanyCheckbox, CheckboxLabel } from '../../Assets/Styles/SummaryPanel/ReservationForm.styles';
  ...
  const handleFetchCompanyData = async (e) => {
    ...
    if (form.nip) {
      try {
        ...
        if (!companyData.subject || (companyData.subject.statusVat !== 'Czynny') || (!companyData.subject.residenceAddress && !companyData.subject.workingAddress)) {
      ...
      <FormBox>
        <Section>
          ...
          <CompanyCheckbox 
            ...
            />
            <CheckboxLabel htmlFor="isCompanyCheckbox">{t("reservationForm.company")}</CheckboxLabel>
            </CheckboxContainer>
          <Label>{t("reservationForm.name")}</Label>
          <Input type="text" name="nameSurname" value={form.nameSurname || ''} onChange={(e) => handleChange(e.target.name, e.target.value)} />
          {errors.nameSurname && <ErrorMessage>{errors.nameSurname}</ErrorMessage>}

          <Label>{t("reservationForm.phone")}</Label>
          <Input type="tel" name="phoneNumber" value={form.phoneNumber} onChange={(e) => handleChange(e.target.name, e.target.value)} />
          {errors.phoneNumber && <ErrorMessage>{errors.phoneNumber}</ErrorMessage>}

          <Label>E-mail:</Label>
          <Input type="email" name="email" value={form.email} onChange={(e) => handleChange(e.target.name, e.target.value)} onBlur={fillUserData} />
          {errors.email && <ErrorMessage>{errors.email}</ErrorMessage>}

          <Label>{t("reservationForm.id-card-licence")}</Label>
          <Input type="text" name="driveNr" value={form.driveNr} onChange={(e) => handleChange(e.target.name, e.target.value)} />
          {errors.driveNr && <ErrorMessage>{errors.driveNr}</ErrorMessage>}

          {form.isCompany && (
            <>
              <LabelNip>{t("reservationForm.nip")}
                <LabelNipLink onClick={handleFetchCompanyData}>
                  {t("reservationForm.fetchCompanyData")}
                </LabelNipLink>
              </LabelNip>
              <Input type="text" name="nip" value={form.nip} onChange={(e) => handleChange(e.target.name, e.target.value)} />
              {errors.nip && <ErrorMessage>{errors.nip}</ErrorMessage>}
              {nipErrorMessage && <ErrorMessage>{nipErrorMessage}</ErrorMessage>}

              <Label>{t("reservationForm.regon")}</Label>
              <Input type="text" name="regon" value={form.regon} onChange={(e) => handleChange(e.target.name, e.target.value)} />
        ...
        <Section>
          ...
          <Input type="text" name="streetNr" value={form.streetNr} onChange={(e) => handleChange(e.target.name, e.target.value)} />
          {errors.streetNr && <ErrorMessage>{errors.streetNr}</ErrorMessage>}

          <Label>{t("reservationForm.zip-code")}</Label>
          <Input type="text" name="zipCode" value={form.zipCode} onChange={(e) => handleChange(e.target.name, e.target.value)} />
          {errors.zipCode && <ErrorMessage>{errors.zipCode}</ErrorMessage>}

          <Label>{t("reservationForm.City")}</Label>
          <Input type="text" name="city" value={form.city} onChange={(e) => handleChange(e.target.name, e.target.value)} />
          {errors.city && <ErrorMessage>{errors.city}</ErrorMessage>}

          <Label>{t("reservationForm.Country")}</Label>
          <Input type="text" name="country" value={form.country} onChange={(e) => handleChange(e.target.name, e.target.value)} />
